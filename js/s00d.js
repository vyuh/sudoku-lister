...
pk.cell.prototype.wait = 0x20;
pk.cell.prototype.open = 0x10;
pk.cell.prototype.data = 0xf;
pk.cell.prototype.may_b = [
...
pk.s00d.prototype.first = function() { // first waiting cell or >=81; for ordered listing of solutions
	var i = 0
	for(; i<81; i++) if(this.i_v[i].yo(pk.cell.wait)) break // pk.cell.wait or this.i_v[i].wait ?
	return i
}
pk.s00d.prototype.best = function() {
	var i=0, level=9, b=81;
	for(; i<81; i++) {
		if(this.i_v[i].yo(pk.cell.wait)) {
...
pk.s00d.prototype.hook = function() { // returns { wrong, stale, solved, dumping } = -1, 0, 1, 2
	...
	while((pos = this.idea()) < 81) {
		...
		for(x=0; x<20; x++) {       
			if(this.i_v[this.clr[pos][x]].rm(val)) return -1
		} // try removing braces
...
class s00d {
    ...
    byte idea() { //first open cell or >=81
        byte i = 0;
        for (; i<81; i++) if (this.i_v[i].yo(cell.open)) break;
        return i;
    }
